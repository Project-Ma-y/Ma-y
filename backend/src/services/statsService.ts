...
export const getSignupConversionService = async () => {
  try {
    ...
    const statsMap = new Map<
      string,
      { sessions: number; registeredSessions: number }
    >();
    ...
    snapshot.forEach((doc: any) => {
      ...
      if (!statsMap.has(date)) {
        statsMap.set(date, { sessions: 0, registeredSessions: 0 });
      }

      const entry = statsMap.get(date)!;
      entry.sessions += 1;
      if (data.isRegistered) {
        entry.registeredSessions += 1;
      }
    })
    ...
    //누적(total) 값 추가
    let totalSessions = 0;
    let totalRegisteredSessions = 0;

    result = result.map((entry) => {
      totalSessions += entry.sessions;
      totalRegisteredSessions += entry.registeredSessions;
      return {
        ...entry,
        totalSessions,
