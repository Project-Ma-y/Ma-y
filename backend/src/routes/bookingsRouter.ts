...
router.get("/admin", verifyFirebaseToken, loadSession, noCache, getAllBookings); //예약 전체 불러오기, 관리자용
router.get("/my", verifyFirebaseToken, loadSession, noCache, getMyBookings); //내 예약만 불러오기
router.get("/:id", verifyFirebaseToken, loadSession, noCache, getBookingById); //특정 예약 조회
router.delete("/:id", verifyFirebaseToken, loadSession, noCache, deleteBooking); //예약 삭제
